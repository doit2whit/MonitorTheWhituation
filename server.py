...
def fetch_calendar_spread():
    ...
    # Brent futures month codes: F=Jan G=Feb H=Mar J=Apr K=May M=Jun
    ...
    try:
        ...
        write_cache("calendar_spread", data)
        return data
    ...
    try:
        ...
        def package_metric(key, data):
            ...
            if "display_multiplier" in meta:
                ...
                display_data = [{"date": d["date"], "value": round(d["value"] / d_val, 1)} for d in data]
